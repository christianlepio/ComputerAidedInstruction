...
<?php
  ...
  $sql = "SELECT COUNT(StudentID) as studcount FROM tblstudent;";
  $res = mysqli_query($conn, $sql);

  if(mysqli_num_rows($res)){
    while($row = mysqli_fetch_assoc($res)){
      $student = $row['studcount'];
    }
  }

  $sql = "SELECT COUNT(USERID) as teachcount FROM tblusers WHERE TYPE = 'Teacher';";
  $res = mysqli_query($conn, $sql);

  if(mysqli_num_rows($res)){
    while($row = mysqli_fetch_assoc($res)){
      $teach = $row['teachcount'];
    }
  }

  $sql = "SELECT COUNT(lessonid) as lessoncount FROM lessontbl";
  $res = mysqli_query($conn, $sql);

  if(mysqli_num_rows($res)){
    while($row = mysqli_fetch_assoc($res)){
      $lescount = $row['lessoncount'];
    }
  }
?>
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      ...
      <div class="row justify-content-center p-3 mb-4 gy-3 gx-5">
        <div class="col-md">
          <div class="d-flex justify-content-between bg-warning p-4 shadow-sm rounded">
            <h5><i class="fa fa-users bg-light p-4 rounded-circle"></i><span class="text-white">&nbsp; Student</span></h5>
            <h1 class="text-white"><?php echo $student;?> &nbsp;</h1>
          </div>
        </div>
        <div class="col-md">
          <div class="d-flex justify-content-between bg-success p-4 shadow-sm rounded">
            <h5><i class="fa-solid fa-chalkboard-user bg-light p-4 rounded-circle"></i><span class="text-white">&nbsp; Teacher</span></h5>
            <h1 class="text-white"><?php echo $teach;?> &nbsp;</h1>
          </div>
        </div>
        <div class="col-md">
          <div class="d-flex justify-content-between bg-primary p-4 shadow-sm rounded">
            <h5><i class="fa fa-book bg-light p-4 rounded-circle"></i><span class="text-white">&nbsp; Lesson</span></h5>
            <h1 class="text-white"><?php echo $lescount;?> &nbsp;</h1>
          </div>
        </div>
      </div>
      <hr>
